#[macro_export]
/// Implements parsing of a [Sequence] of unnamed values into a named struct. e.g. (1 2 3) => struct Point<'a>{x: i32, y: usize, z: &'a str }
///
/// # Errors
///
/// - Returns `Err(String)` if a required delimiter is not found
/// - Returns `Err(String)` if extra elements remain after parsing all fields
///
/// # Example
///
/// ```ignore
/// auto_impl_sequence! {
///     pub struct Point(pub i32: x [','], pub i32: y);
/// }
/// ```
macro_rules! auto_impl_sequence {
    ...
    (
        ...
        $vis struct $name$(<$($g),*>)?{$( $visf $idf: $tyf ),*}
